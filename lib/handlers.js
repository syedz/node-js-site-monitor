...
handlers.index = function(data, callback) {
    ...
    if (data.method == 'get') {
        ...
        var templateData = {
            ...
            'head.description': 'Offering free upsite monitor via SMS',
            'body.class': 'index'
        };
        ...
        _data.read('tokens', id, function(err, tokenData) {
            if (!err && tokenData) {
                ...
                if (tokenData.expires > Date.now()) {
                    ...
                    callback(400, {
                        Error: 'The token has alrady expired, and cannot be extend'
                    });
                }
            } else {
                callback(400, {
                                ...
                                if (!err) {
                                    ...
                                    _data.update('users', userPhone, userData, function(err) {
                                        if (!err) {
                                            ...
                                            callback(500, {
                                                Error: 'Coud not update the user with the new check'
                                            });
                                        }
                                    });
                                } else {
    ...
    if (id) {
        ...
        _data.read('checks', id, function(err, checkData) {
            if (!err && checkData) {
                // Get the token from the headers
                var token = typeof data.headers.token == 'string' ? data.headers.token : false;
                // Verify that the given token is valid and belong to the user whoe created the check
...
// Checks - put
// Required data: id
// Optional data: protocol, url, method, successCodes, timeouSeconds (one must be sent)
                    ...
                    if (tokenIsValid) {
                        ...
                        _data.delete('checks', id, function(err) {
                            if (!err) {
                                ...
                                _data.read('users', checkData.userPhone, function(err, userData) {
                                    if (!err && userData) {
                                        ...
                                        if (checkPosition > -1) {
                                            ...
                                            callback(500, {
                                                Error:
                                                    "Could not find the check on the user's objects, so could not remove it"
                                            });
                                        }
                                    } else {
                                        callback(500, {
                                ...
                                callback(500, {
                                    Error: 'Coud not delete the data;'
                                });
                            }
                        });
                    } else {
